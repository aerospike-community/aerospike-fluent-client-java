...
public class SystemSettings {
    ...
    /**
     * Applies these system settings to a ClientPolicy.
     * 
     * @param policy the ClientPolicy to configure
     */
    public void applyTo(ClientPolicy policy) {
        if (minimumConnectionsPerNode != null) {
            policy.minConnsPerNode = minimumConnectionsPerNode;
        }
        if (maximumConnectionsPerNode != null) {
            policy.maxConnsPerNode = maximumConnectionsPerNode;
        }
        if (maximumSocketIdleTime != null) {
            policy.maxSocketIdle = (int) maximumSocketIdleTime.toSeconds();
        }
        if (numTendIntervalsInErrorWindow != null) {
            policy.errorRateWindow = numTendIntervalsInErrorWindow;
        }
        if (maximumErrorsInErrorWindow != null) {
            policy.maxErrorRate = maximumErrorsInErrorWindow;
        }
        if (tendInterval != null) {
            policy.tendInterval = (int) tendInterval.toMillis();
        }
    }
