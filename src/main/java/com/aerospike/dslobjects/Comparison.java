...
public class Comparison<T> implements BooleanExpression {
    ...
    private Exp rightAsExp() {
        if (right instanceof DslExpression) {
            ...
            if (right instanceof String) return Exp.val((String)right);
            if (right instanceof Double) return Exp.val((Double)right);
            if (right instanceof Float) return Exp.val((Float)right);
            if (right instanceof Integer) return Exp.val((Integer)right);
            if (right instanceof Long) return Exp.val((Long)right);
            if (right instanceof byte[]) return Exp.val((byte [])right);
            if (right instanceof Boolean) return Exp.val((Boolean)right);
        }
