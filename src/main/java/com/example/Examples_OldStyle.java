...
public class Examples_OldStyle {
    ...
    public long run() {
        IAerospikeClient connection = new AerospikeClient("172.17.0.2", 3100);
        ...
        connection.truncate(null, "test", "testSet", null);
        WritePolicy wp = new WritePolicy(writePolicy);
        wp.recordExistsAction = RecordExistsAction.UPDATE;
        connection.put(null, key3, new Bin("person", getMapToStore()), new Bin("name", "Tim"));
...
//        });
        ...
        connection.put(wp, key1, new Bin("name", "Tim"));
        connection.operate(null, null, new Key[] {key2, key3}, Operation.put(new Bin("name", "Jake")));
        Record[] recs = connection.get(null, new Key[] {key1, key2, key3, key4});
        for (Record r : recs) {
            System.out.println(r);
        }
        Policy policy = new Policy(connection.getReadPolicyDefault());
        policy.maxRetries = 5;
        System.out.println(connection.get(policy, key1));
        
        long now = System.nanoTime();
        for (int i = 0; i < ITERATIONS; i++) {
            System.out.println(connection.get(null, key3));
            connection.operate(wp, key3, 
            ...
            System.out.println(connection.get(null, key3));
            connection.put(wp, key3, Bin.asNull("name"));
            System.out.println(connection.get(null, key3));
            ...
            bwp.sendKey = true;
            connection.operate(batchPolicy, bwp, new Key[] {key1, key2, key3}, Operation.put(new Bin("now", new Date().getTime())));
            connection.put(wp, key1, new Bin("name", "Fred"));
            connection.put(wp, key1, new Bin("name", "Fred"), new Bin("age", 21), Bin.asNull("other"));

            connection.put(wp, key1, new Bin("name", "Fred"), new Bin("age", 21));
            connection.put(wp, key1, new Bin("name", "Alex"), new Bin("age", 99));
            
            BatchPolicy batchReadPolicy = new BatchPolicy(batchPolicy);
            Record[] records = connection.get(batchReadPolicy, new Key[] {key1, key2});
